...
function buildOpenCodePrompt(outcome: Outcome, instruction: string): string {
  ...
  return `You are implementing an Akodo outcome in the current local project.\n\nOutcome: ${outcome.name}\nGoal: ${outcome.goal || outcome.name}\nConstraints:\n${constraints}\nAcceptance criteria:\n${criteria}\n\nImplementation instruction:\n${instruction}\n\nScope boundary: work only inside the current outcome workspace. Do not read, modify, create, or delete files outside this selected project folder. Do not navigate to parent folders, the user home, or other projects.\n\nValidation boundary: do not start a development server, preview server, Electron process, Playwright browser, or any long-running process. In particular, do not run npm run dev, npm run dev:vite, vite, or equivalent commands. Akodo runs project checks and isolated browser validation after implementation on its own ports. You may run short-lived static checks such as typecheck, lint, or test only when they are relevant and guaranteed to exit. Do not create temporary validation scripts.\n\nInspect the project first, implement only what is needed, run relevant short-lived local checks, and summarize the changes and commands you ran. Do not commit, push, or deploy.`;
}
